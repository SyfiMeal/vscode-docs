...
Claude agent sessions now emit OpenTelemetry traces that follow the [GenAI semantic conventions](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/gen-ai/), so you can monitor agent behavior, latency, and token usage in any OTLP-compatible backend (for example, the [Aspire Dashboard](https://aspire.dev/dashboard/standalone/)) that your team might already be using.
